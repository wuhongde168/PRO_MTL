  ...
  <div >
    <div class="cont">
      ...
      <div class="content_list">
        <!-- 循环内容 -->
        <div v-for="(searchs,index) of search_data.slice(0,8)"  :key="index">
          <router-link target="_blank" :to='`/deteils/${searchs.pid}`'  class="link_list">
            <div class="inner">
              <div class="inner_top">
                <img class="pic" :src="searchs.pic" alt="">
                <div class="inner_t">
                  <p class="inner_title">{{searchs.titel}}</p>
                  <p class="issell">售票中</p>
                  <div class="inner_b">
                    <div>
                      <p>{{searchs.act_time}}</p>
                      <p>{{searchs.act_site}}</p>
                    </div>
                    <div class="inner_price"> 
                      <span>{{searchs.price}}</span><span>元起</span>
                    </div>
                  </div>
                </div>
              </div> 
              <div class="inner_foot">
                <div class="browse">
                  ...
                  <span>{{searchs.pv}}</span><span>人浏览</span>
                </div>
                <div class="attention">
                  <img src="../assets/icons/attention.svg" alt="">
                  <span>{{searchs.attention}}</span><span>人想看</span>
                </div>
              </div>
            </div>
          </router-link>
...
export default {
  ...
  mounted(){
    let text= this.$route.params.text;
      this.axios.get(`/search?text=${text}`).then(res=>{
